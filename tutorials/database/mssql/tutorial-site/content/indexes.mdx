...
1. Non-Clustered Index
    ...
    A Non-clustered index creates a separate index table to store the sorted column values and a pointer reference to the records called the ROWID.
    A ROWID is a unique hexadecimal value that stores the actual physical address of a row.

Let's understand the concept of indexes with an example.
...
Solution: Create a PK on ID table
...
The query analyzer now seeks for the ID in the clustered index and scans the column 'ID' alone. Number of rows read is just 1 in this case. Also, the estimated subtree cost now reduced to 0.003281.
...
You can also use a combination of columns. For example to sort the records based on the Gender (descending) column, followed by the Salary(ascending) column.
...
Execution plan is read from right to left and top to bottom.
...
1. The primary key clustered index does a key lookup for the row that with the ROWID pointer address.
...
1. The final select query prints the record with an improved Subtree cost from 1.09254 to 0.00657.
    ...
    ![Subtree cost after non-clustered index](../../../../assets/non-clustered-subtree-cost-improved.png)
